...
namespace Checkers_WinForms
{
    class Player1
    {
        ...
        public static Bitmap checkerImage = Properties.Resources.Player1Tool;
        public static Bitmap kingCheckerImage = Properties.Resources.Player1Crown;
        ...
        public bool HasToJump(PictureBox[,] board)
        {
            foreach (Checker checker in playerCheckers)
            {
                if (!checker.IsKing)
                {
                    if (checker.Position.Y < 6)
                    {
                        if (checker.Position.X <= 1)
                        {
                            if (board[checker.Position.X + 2, checker.Position.Y + 2].Image == null)
                            {
                                if (board[checker.Position.X + 1, checker.Position.Y + 1].Image == Player2.checkerImage || board[checker.Position.X + 1, checker.Position.Y + 1].Image == Player2.kingCheckerImage)
                                    return true;
                            }
                        }
        ...
        internal bool IsValidJump(Checker selectedChecker, Point clickedLocation, PictureBox[,] newBoard)
        {
            if (!selectedChecker.IsKing)
            {
                if (clickedLocation.Y - selectedChecker.Position.Y == 2)
                {
                    ...
                    {
                        if (newBoard[clickedLocation.X - 1, clickedLocation.Y - 1].Image == Player2.checkerImage || newBoard[clickedLocation.X - 1, clickedLocation.Y - 1].Image == Player2.kingCheckerImage)
                            return true;
                    }
                    if (clickedLocation.X - selectedChecker.Position.X == -2)
                    {
        ...
        internal void ExecuteJump(Checker selectedChecker, Point clickedLocation, PictureBox[,] newBoard, Player2 player2)
        {
            newBoard[selectedChecker.Position.X, selectedChecker.Position.Y].Image = null;
            if (clickedLocation.X - selectedChecker.Position.X == 2 && clickedLocation.Y - selectedChecker.Position.Y == 2)
            {
                player2.RemoveCheckerFromList(new Point(clickedLocation.X - 1, clickedLocation.Y - 1));
        ...
        {
            if (selectedChecker.Position.Y == 7)
                return true;
            return false;
        }
    }
